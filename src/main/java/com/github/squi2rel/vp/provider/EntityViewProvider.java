...
public class EntityViewProvider implements IVideoProvider {
    ...
    public @Nullable CompletableFuture<VideoInfo> from(String str, IProviderSource source) {
        if (REGEX.matcher(str).matches()) return CompletableFuture.completedFuture(new VideoInfo(source.name(), "ENTITY VIEW", "", str, -1, false, NO_PARAMS));
        ...
        return CompletableFuture.completedFuture(new VideoInfo(source.name(), "ENTITY VIEW", "", player.getUuidAsString(), -1, false, NO_PARAMS));
    }
}
